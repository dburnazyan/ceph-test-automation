...
def check_output_ssh(host, opts, cmd):
    ...
    return check_output("sshpass -p {4} ssh {2} {3}@{0} {1}".format(host, cmd, SSH_OPTS, SSH_USER, SSH_PASSWORD), False)
...
class CephPerformanceCollector(Collector):
    ...
    def start_performance_monitoring(self, path, host, osd_devs):
        ...
        try:
            scp_cmd = "sshpass -p {5} scp {0} {1} {4}@{2}:{3}".format(SSH_OPTS, local_file,
                                                   host, self.remote_file, SSH_USER, SSH_PASSWORD)
...
def get_sshable_hosts(hosts, thcount=32):
    cmd = "sshpass -p "+SSH_PASSWORD+" ssh " + SSH_OPTS + " -o ConnectTimeout=5 -o ConnectionAttempts=1 "

    def check_host(host):
        ok, out = check_output(cmd + SSH_USER+"@"+ host + ' pwd')
        if ok:
            return host
